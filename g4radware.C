...
void genBackground(int res=10,int nosmoothing=1);
void write();

TString foldername;

void g4radware(TString name = "root")
{
  foldername = name;
    
  TFile *fin = new TFile(Form("%s/g4hist.root",foldername.Data()));
  if ((!fin) || (fin->IsZombie())) {
    cout<<Form("%s/g4hist.root",foldername.Data())<<" does not exist!"<<endl;
    return;
  }
  ...
  //matrix
  hg2xyb = new TH2F("hg2xyb","radware background matrix",nbin,xmin,xmax,nbin,xmin,xmax);//background matrix
...
void write()
{
  ...
  //output to ROOT file
  TFile *fout = new TFile(Form("%s/g4radware.root",foldername.Data()),"RECREATE");//user
  hg2xy->SetName("hg2xy");
  hg2xy->Write();
  hg2xyp->Write();
  hg2x->Write();
  hg2xyb->Write();
  fout->Close();
  cout<<"Write Histgrams hg2xyp, hg2x and hg2xb to "<<Form("%s/g4radware.root",foldername.Data())<<endl;
  gROOT->ProcessLine(".q");
}
